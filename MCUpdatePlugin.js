...
export class MCUpdatePlugin extends plugin {
		...
		try {
			...
			if (newVersions.length > 0) {
				...
				newVersions.forEach((v) => {
					...
					message += `\n▸ 类型：${v.type}`
					message += `\n▸ 发布时间：${v.releaseTime}`
					message += `\n▸ 更新日志：${this.getMinecraftUpdateLogUrl(v.id)}\n`
				})
	...
	getMinecraftUpdateLogUrl(versionId) {
		const baseUrl = 'https://www.minecraft.net/zh-hans/article'
		const version = `minecraft-${versionId.toLowerCase().replace(/\./g, '-')}`
	...
	async manualCheck(e) {
		try {
			...
			message += `▸ 更新日志：${this.getMinecraftUpdateLogUrl(latest.id)}`
